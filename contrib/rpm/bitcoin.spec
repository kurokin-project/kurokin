...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9889
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 9888
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 10889
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 10888
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 9889
	%{_sbindir}/semanage port -d -p tcp 9888
	%{_sbindir}/semanage port -d -p tcp 10889
	%{_sbindir}/semanage port -d -p tcp 10888
	for selinuxvariant in %{selinux_variants}; do
