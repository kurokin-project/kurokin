Kurokin Core version 1.0.0.8 is now available from:
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/kurokin-project/kurokin/issues>
...
updates will be released anymore. Without security updates, using a kurokin
...
ECDSA signatures inside Kurokin transactions now use validation using
...
Previous versions of Kurokin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Kurokin Core was to raise the default minimum
relay fee.

Kurokin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Kurokin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running kurokind without having to do any manual configuration.
...
Kurokin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Kurokin Core will relay transactions with
...
entirely in Kurokin Core 0.13, and it is at this time undecided whether the
...
Kurokin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Kurokin Core automatically creates a hidden service to listen on, without
manual configuration. Kurokin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Kurokin Core is listening, and
...
Kurokind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Kurokin
Core to use floating fees. By default, Kurokin Core will use floating
fees.
...
(default: `0.0002` KURO/kB).

At all times, Kurokin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) KURO.
Furthermore, Kurokin Core will never create transactions paying less than
...
`kurokin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 29889 to a RPC server bound on localhost on port 19884 do:

    stunnel -d 29889 -r 127.0.0.1:19884 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /kurokinrpc>
        ...
        # optional bypass kurokind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< kurokinrpc:<password>
    </Location>
...
Restarting kurokind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in kurokin.conf
...
For example:
    ...
    $ src/kurokin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Kurokin Core code base has
...
- `kurokin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `kurokin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Kurokin scripting system that in combination with
...
Kurokin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Kurokin Core.
...
The two phases are distinct in the Kurokin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Kurokin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Kurokin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Kurokin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of kurokins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of kurokins being spent is exactly
...
- **Increased security for multisig:** Kurokin addresses (both P2PKH addresses
  ...
  believed to be provided by Kurokin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Kurokin's current ledger (called the UTXO set),
  ...
  note that Kurokin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Kurokin users to individually opt-in to almost any change in the Kurokin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Kurokin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Kurokin
client to Kurokin Core 0.13.2 to signal for segwit support. When a
...
Since Kurokin Core 0.10.0, nodes have defaulted to only relaying and
...
- `kurokin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `kurokin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
